...
const SpawnParameterData = VehicleData.SpawnParameterData
...
func _bind_cells(next_parameter: ParameterData):
	...
	source.bind(&"vehicle_spawn_rate_before_start").to(%SpawnRateBeforeStartBox, &"value", &"value_changed")
	source.bind(&"vehicle_spawn_rate_after_start").to(%SpawnRateAfterStartBox, &"value", &"value_changed")

	source.bind(&"vehicle_spawn_parameters").to(_class_manager, &"parameters", _class_manager.parameters_changed)
...
func _unbind_cells(prev_parameter: ParameterData):
	...
	source.unbind(&"vehicle_spawn_rate_before_start").from(%SpawnRateBeforeStartBox, &"value")
	source.unbind(&"vehicle_spawn_rate_after_start").from(%SpawnRateAfterStartBox, &"value")

	source.unbind(&"vehicle_spawn_parameters").from(_class_manager, &"parameters")
