...
signal value_inserted(position: int, added_value: Variant)
signal value_removed(position: int, removed_value: Variant)
signal duplicated
...
func append(value: Variant) -> void:
	...
	_data.append(value)
	value_inserted.emit(position, value)
...
func append_array(array: Array) -> void:
	...
	for offset in range(len(array)):
		...
		value_inserted.emit(position, added_value)
...
func assign(array: Array) -> void:
	...
	for position in range(len(clone)):
		...
		value_removed.emit(position, removed_value)
...
func erase(value: Variant) -> void:
	...
	value_removed.emit(position, value)
...
func pop_back() -> Variant:
	...
	value_removed.emit(len(_data), value)
	return value


func pop_front() -> Variant:
	var value = _data.pop_front()
	value_removed.emit(0, value)
	return value
...
func resize(new_size: int) -> int:
	...
	if old_size < new_size:
		for position in range(old_size, new_size):
			var emitter = value_inserted.emit.bind(position, null)
			emitters.append(emitter)

	else:
		for position in range(new_size, old_size):
			var value = _data[position]
			var emitter = value_removed.emit.bind(position, value)
			emitters.append(emitter)
